"""Re-prep sisada Chifón sheets for the luminance tint compositor.

v8.2.1/v8.2.2 left both tank sheets too light (median ~0.75 vs the 0.66
tint reference) with almost no fold contrast. White / #F4F3EF Cuerpo then
clips toward paper-white and the silhouette disappears on the studio bg.

Interior pixels are made fully opaque. Luminance is scaled to 0.66 and
highlights above 0.80 are compressed, matching the zipneck harden. Runtime
hardenSleevelessPhoto adds edge/form shading and a light-body contrast boost.
The tank silhouettes are unchanged.
"""
...
SHEETS = (
    ROOT / 'chifon-sleeveless-hombre-front-back.png',
    ROOT / 'chifon-sleeveless-mujer-front-back.png',
)
TARGET_MEDIAN = 0.66
...
def main():
    for src_path in SHEETS:
        src = np.array(Image.open(src_path))
        out, info = prep(src)
        Image.fromarray(out, 'RGBA').save(src_path, optimize=True)
        print('Wrote', src_path.name, info)
