...
function jerseyChifonFamily(g=state.garment){return g==='playera'||g==='zipneck'||g==='sleeveless'}
function fabricSkuOptions(){
  if(state.garment==='polo')return POLO_FABRIC_SKUS;
  if(jerseyChifonFamily())return [
    {value:'CHIFÓN 140',label:'CHIFÓN 140 · Chifón'},
    {value:'CHIFÓN ESTRELLA',label:'CHIFÓN ESTRELLA · Chifón Estrella'}
  ];
  return [];
}
...
function syncFabricSkuList(){
  ...
  fabric.placeholder=state.garment==='polo'?'Piqué Olmo, Atlante o Fomer':jerseyChifonFamily()?'Chifón / Chifón Estrella':'Ej. jersey de algodón';
}
